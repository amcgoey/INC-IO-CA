...
- **GAS Compatibility & Tier Boundary Leak** — Node built-in imports (`fs`, `path`, `crypto`) inside `src/` (Tier 1/2) or GAS globals (`SpreadsheetApp`, `DriveApp`) leaked into Tier 1 pure domain logic, or legacy `typeof require !== 'undefined'` guards and manual `globalThis` boilerplate in internal `src/` files. → use clean ES modules (`import`/`export`) and move GAS calls to Tier 2 infrastructure adapters behind Tier 1 interfaces.

### 4. Spawn both sub-agents in parallel

Use the `invoke_subagent` tool to spawn both sub-agents in a single tool call. Set the `Model` argument to `pro` for both to ensure they use the latest Pro model with High thinking.

...
- **GAS Compatibility & Tier Boundary Leak** — Node built-in imports (`fs`, `path`, `crypto`) inside `src/` (Tier 1/2) or GAS globals (`SpreadsheetApp`, `DriveApp`) leaked into Tier 1 pure domain logic. → move GAS calls to Tier 2 infrastructure adapters behind Tier 1 interfaces.

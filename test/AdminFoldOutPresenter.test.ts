...
describe("AdminFoldOutPresenter & SheetAdminFoldOut (Issue #220, #224)", () => {
  ...
  function convertWorkbookSpecToBatchData(workbookSpec: DocumentLogWorkbookSpec, spreadsheetId: string): SpreadsheetBatchData {
    const sheetMap = new Map<string, number>();
    const sheets = workbookSpec.tabs.map((tab, idx) => {
      const sheetId = idx + 1;
    ...
    const namedRanges = workbookSpec.namedRanges.map((nr) => {
      ...
      const parsedRange = parseRangeNotation(nr.rangeNotation || nr.range || "A1:Z100");
      return {
        name: nr.name,
        range: {
          sheetId,

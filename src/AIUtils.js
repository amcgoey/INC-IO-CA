 ...
 */
function getCachedLibrary_(url) {
  const cache = CacheService.getScriptCache();
  let script = cache.get("pdf_lib_script");
  
  if (!script) {
    script = UrlFetchApp.fetch(url).getContentText();
    try {
      // Cache for 10 minutes (600 seconds)
      cache.put("pdf_lib_script", script, 600);
    } catch (e) {
...
function getCachedPrediction(messageId) {
  if (!messageId) return null;
  const cache = CacheService.getUserCache();
  const cached = cache.get(`ai_pred_${messageId}`);
  return cached ? JSON.parse(cached) : null;
}

function setCachedPrediction(messageId, predictionObj) {
  if (!messageId) return;
  const cache = CacheService.getUserCache();
 ...
 */
function getCachedDrives() {
  const cache = CacheService.getUserCache();
  const cached = cache.get("cached_shared_drives");
  ...
  try {
    let pageToken;
    do {
      let resp = Drive.Drives.list({ maxResults: 100, pageToken: pageToken, fields: "items(id,name),nextPageToken" });
      if (resp.items) {
        drives = drives.concat(resp.items.map(d => ({ id: d.id, name: d.name })));
      }
  ...
  if (drives.length > 0) {
    try {
      cache.put("cached_shared_drives", JSON.stringify(drives), 21600); // Cache for 6 hours
    } catch (e) {
...
function getAvailableDriveNames() {
  const drives = getCachedDrives();
  return drives.map(d => d.name);
}

function fetchGeminiWithRetry(url, options, maxRetries = 2) {
  let attempts = 0;
...
function predictProjectAndDiscipline(emailData, driveNames) {
...
function fetchAndSaveFile(url, folderId) {
  try {
    ...
    if (blob.getContentType().includes('text/html')) return { success: false, error: "AUTH_WALL" };
    
    const file = DriveApp.getFolderById(folderId).createFile(blob.setName("Fetched_Submittal.pdf")); 
    return { success: true, fileId: file.getId(), fileName: file.getName() };
  } catch (err) {
    if (err.message.includes("not been whitelisted")) return { success: false, error: "NOT_WHITELISTED" };
    return { success: false, error: sanitizeErrorString(err.message) };
  }
}

async function splitPdfToBase64(sourceBlob) {
...
async function analyzeSubmittalDeep(sourceBlob, emailText, contextObj) {
  const apiKey = getGeminiApiKey();

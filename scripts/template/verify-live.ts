...
export async function runLiveVerification(
  ...
  if (!opts.dryRun && token && fetcher) {
    try {
      ...
      const appendUrl = `https://sheets.googleapis.com/v4/spreadsheets/${opts.spreadsheetId}/values/'Submittal Arch'!A20:Z20:append?valueInputOption=USER_ENTERED`;
      const rowValues = [
        sampleRow.section,
        sampleRow.number,
        sampleRow.title,
      ...
      const readUrl = `https://sheets.googleapis.com/v4/spreadsheets/${opts.spreadsheetId}/values/'Submittal Arch'!K20:O20?valueRenderOption=FORMATTED_VALUE`;
